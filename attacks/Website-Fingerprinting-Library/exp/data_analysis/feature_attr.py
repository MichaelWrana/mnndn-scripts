...
train_X, train_y = data_processor.load_data(os.path.join(in_path, f"{args.in_file}.npz"), args.feature, args.seq_len)
...
model = eval(f"models.{args.model}")(num_classes)
model.load_state_dict(torch.load(os.path.join(ckp_path, f"{args.save_name}.pth"), map_location=device))
model.to(device)
...
attr_values = analyzer.feature_attr(model, args.attr_method, train_X, train_y, num_classes)
